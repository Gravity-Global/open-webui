...
resource "aws_security_group" "ecs_scaled_sg" {
  name_prefix = "webui-ecs-scaled-"
  vpc_id      = var.vpc_id
  description = "Security group for WebUI scaled ECS tasks"
  ...
  tags = {
    Name = "WebUI ECS Scaled Security Group"
  }
}
...
resource "aws_ecs_task_definition" "webui_scaled" {
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        {
          name  = "REDIS_URL"
          value = "rediss://:${urlencode(random_password.redis_auth_token.result)}@${aws_elasticache_replication_group.redis.primary_endpoint_address}:${aws_elasticache_replication_group.redis.port}"
        },
        {
  ...
  tags = {
    Name = "WebUI Scaled Task Definition"
  }
}
