...
resource "aws_security_group" "alb_sg" {
  name_prefix = "webui-alb-"
  vpc_id      = var.vpc_id
  description = "Security group for WebUI Internal ALB"
  ...
  tags = {
    Name = "WebUI ALB Security Group"
  }
}

# Internal Application Load Balancer
resource "aws_lb" "webui_alb" {
  name               = "webui-internal-alb"
  internal           = true
...
resource "aws_s3_bucket" "alb_logs" {
  bucket        = "webui-alb-logs-${random_id.bucket_suffix.hex}"
  force_destroy = true

  tags = {
    Name = "WebUI ALB Access Logs"
  }
}
    ...
    Statement = [
      {
        ...
        Action   = "s3:PutObject"
        Resource = "${aws_s3_bucket.alb_logs.arn}/webui-alb/AWSLogs/${data.aws_caller_identity.current.account_id}/*"
      },
      {
        Effect = "Allow"
        Principal = {
